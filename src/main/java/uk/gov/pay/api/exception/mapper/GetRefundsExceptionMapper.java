...
import static uk.gov.pay.api.model.PaymentErrorCodes.GET_PAYMENT_REFUNDS_CONNECTOR_ERROR;
import static uk.gov.pay.api.model.PaymentErrorCodes.GET_PAYMENT_REFUNDS_NOT_FOUND_ERROR;
...
public class GetRefundsExceptionMapper implements ExceptionMapper<GetRefundsException> {
    ...
    public Response toResponse(GetRefundsException exception) {
        ...
        if (exception.getErrorStatus() == NOT_FOUND.getStatusCode()) {
            paymentError = aPaymentError(GET_PAYMENT_REFUNDS_NOT_FOUND_ERROR);
            status = NOT_FOUND;
        } else {
